...
class AIReleaseValidator:
    ...
    def _prepare_analysis_context(self, 
        ...
        # Add column historical data with specific focus on recent changes
        if column_historical is not None:
            ...
            for (theme, type_), group in column_historical.groupby(['theme', 'type']):
                ...
                attribute_changes = {'theme': theme, 'type': type_, 'changes': []}
                ...
                # Calculate change between latest and previous
                total_latest = latest['total_count']
                total_prev = previous['total_count']
                
                for col in latest.index:
                    if col not in ['release', 'theme', 'type', 'total_count']:
                        try:
                            val_latest = float(latest[col])
                            val_prev = float(previous[col])
                            
                            cov_latest = (val_latest / total_latest * 100) if total_latest > 0 else 0
                            cov_prev = (val_prev / total_prev * 100) if total_prev > 0 else 0
                            
                            abs_change = cov_latest - cov_prev
                            rel_change = (cov_latest / cov_prev) if cov_prev > 0 else float('inf')
                            
                            # Flag significant changes
                            if abs(abs_change) > 5 or rel_change > 2 or rel_change < 0.5:
                                attribute_changes['changes'].append({
                                    'attribute': col,
                                    'previous_coverage': round(cov_prev, 2),
                                    'current_coverage': round(cov_latest, 2),
                                    'absolute_change': round(abs_change, 2),
                                    'relative_change': round(rel_change, 2) if rel_change != float('inf') else 'new_attribute'
                                })
                        except:
                            pass
                
                context['historical_data'].append(coverage_data)
                if attribute_changes['changes']:
                    context['attribute_changes'].append(attribute_changes)
...
## Key Attribute Changes (Latest Release vs Previous)
These are significant attribute coverage changes that warrant attention:
{json.dumps(context['attribute_changes'], indent=2)}
...
- **CRITICAL**: Pay special attention to attribute coverage changes in the historical data
  - Look for sudden spikes (>2x increase) or drops (>10% decrease)
  - Even small attributes (like emails, socials) matter if they show dramatic changes
  - Compare the most recent release to the previous release for each attribute
