...
def analyze_attribute_coverage(attr_stats: Dict[str, pd.DataFrame],
    ...
    key_attributes = {
        'places': ['names', 'websites', 'phones', 'categories', 'confidence'],
        'buildings': ['names', 'height', 'level', 'class'],
    ...
    for key, df in attr_stats.items():
        ...
        # Check for attribute columns (they end with _count)
        count_cols = [col for col in df.columns if col.endswith('_count') and 
                     col.replace('_count', '') in key_attributes[theme]]
        
        if not count_cols:
            continue
        
        # Calculate coverage percentages
        total_records = df['id'].iloc[0] if 'id' in df.columns else None
        
        if total_records:
            for col in count_cols:
                attr_name = col.replace('_count', '')
                count = df[col].iloc[0]
...
def detect_attribute_drops(comparisons: Dict[str, pd.DataFrame],
    ...
    for key, df in comparisons.items():
        ...
        # Get attribute rows (they have non-null release values)
        attr_rows = df[df[current_release].notna() & df[previous_release].notna()]
        
        for _, row in attr_rows.iterrows():
            current_val = row[current_release]
            previous_val = row[previous_release]
            
            if previous_val > 0:
                pct_change = ((current_val - previous_val) / previous_val) * 100
                
                if pct_change < -threshold:
                    attr_name = row.get('attribute', 'unknown')
...
def run_enhanced_agent(api_key: str, 
    ...
    attr_drop_anomalies = detect_attribute_drops(comparisons, current_release, previous_release)
